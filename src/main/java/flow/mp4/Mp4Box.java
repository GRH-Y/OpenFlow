...
public class Mp4Box {
    ...
    public Mp4Box(File mp4) {
        try {
            mp4File = new RandomAccessFile(mp4, "r");
...
//                LogDog.i("==> srcData " + getClass().getName() + " = " + StringUtils.byteToHexStr(srcData));
        ...
        protected int[] parseArray(int startIndex) {
            int[] array = new int[(srcData.length - startIndex) / 4];
            int count = 0;
            for (int index = startIndex; index < srcData.length; index += 4) {
...
//    }
    ...
    public class SttsBox extends Box {
        ...
        @Override
        protected void parseSub() throws Exception {
            numCount = TypeConversion.byteToLong(srcData);
            sttsList = new ArrayList<>((int) numCount);
            for (int index = 8; index < srcData.length; index += 4) {
                Stts stco = new Stts();
                stco.sampleCount = TypeConversion.byteToInt(srcData, index);
                index += 4;
                stco.sampleDelta = TypeConversion.byteToInt(srcData, index);
                sttsList.add(stco);
            }
        }
    ...
    public class CttsBox extends Box {
        ...
        protected void parseSub() throws Exception {
            numCount = TypeConversion.byteToLong(srcData);
            cttsList = new ArrayList<>((int) numCount);
            for (int index = 8; index < srcData.length; index += 4) {
                Ctts ctts = new Ctts();
                ctts.sampleCount = TypeConversion.byteToInt(srcData, index);
                index += 4;
                ctts.sampleOffset = TypeConversion.byteToInt(srcData, index);
                cttsList.add(ctts);
            }
        }
    ...
    public class StssBox extends Box {
        ...
        public List<StssBox.Stss> stssList;
        public long numCount = 0;
        ...
        protected void parseSub() throws Exception {
            numCount = TypeConversion.byteToLong(srcData);
            stssList = new ArrayList<>((int) numCount);
            for (int index = 8; index < srcData.length; index += 4) {
                Stss stsc = new Stss();
                stsc.sampleNumber = TypeConversion.byteToInt(srcData, index);
                stssList.add(stsc);
            }
            stssArray = parseArray(0);
        }
    ...
    public class StscBox extends Box {
        ...
        protected void parseSub() throws Exception {
            numCount = TypeConversion.byteToLong(srcData);
            stscList = new ArrayList<>((int) numCount);
            for (int index = 8; index < srcData.length; index += 4) {
                Stsc stsc = new Stsc();
    ...
    public class StszBox extends Box {
        ...
        @Override
        protected void parseSub() throws Exception {
            sampleSize = TypeConversion.byteToInt(srcData, 0);
            sampleCount = TypeConversion.byteToLong(srcData, 4, 12);
            stszList = new ArrayList<>((int) sampleCount);
            for (int index = 12; index < srcData.length; index += 4) {
                Stsz stco = new Stsz();
                stco.sampleSize = TypeConversion.byteToInt(srcData, index);
                stszList.add(stco);
            }
        }
    ...
    public class StcoBox extends Box {
        ...
        protected void parseSub() throws Exception {
            numCount = TypeConversion.byteToLong(srcData);
            stcoList = new ArrayList<>((int) numCount);
            for (int index = 8; index < srcData.length; index += 4) {
                Stco stco = new Stco();
                stco.sampleSize = TypeConversion.byteToInt(srcData, index);
                stcoList.add(stco);
